...
use util_libs:: nats_js_client::{get_nats_creds_by_nsc, get_file_path_buf};
...
impl Keys {
    pub fn new() -> Result<Self> {
        ...
        let auth_guard_creds = get_file_path_buf(&get_nats_creds_by_nsc("HOLO", "AUTH", "auth_guard"));
    ...
    pub fn try_from_storage(maybe_host_creds_path: &Option<PathBuf>, maybe_sys_creds_path: &Option<PathBuf>) -> Result<Self> {
        let host_key_path = std::env::var("HOST_KEY_PATH").context("Cannot read HOST_KEY_PATH from env var")?;
        let host_keypair = try_read_keypair_from_file(get_file_path_buf(&host_key_path.clone()))?.ok_or_else(|| anyhow!("Host keypair not found at path {:?}", host_key_path))?;
        let host_pk = host_keypair.public_key();
        let sys_key_path = std::env::var("SYS_KEY_PATH").context("Cannot read SYS_KEY_PATH from env var")?;
        let host_creds_path = maybe_host_creds_path.to_owned().unwrap_or_else(|| get_file_path_buf(
            &get_nats_creds_by_nsc("HOLO", "HPOS", "host")
        ));
        let sys_creds_path = maybe_sys_creds_path.to_owned().unwrap_or_else(|| get_file_path_buf(
            &get_nats_creds_by_nsc("HOLO", "HPOS", "sys")
        ));
    ...
    pub fn _add_local_sys(mut self, sys_key_path: Option<PathBuf>) -> Result<Self> {
        let sys_key_path = sys_key_path.unwrap_or_else(|| get_file_path_buf(
            &get_nats_creds_by_nsc("HOLO", "HPOS", "sys")
        ));
    ...
    pub fn add_creds_paths(mut self, host_creds_file_path: PathBuf, sys_creds_file_path: Option<PathBuf>) -> Result<Self> {
            ...
            Ok(is_ok) => {
                ...
                let creds = match sys_creds_file_path {
                    Some(sys_path) => match sys_path.try_exists() {
                        ...
                        Err(e) => {
                            return Err(anyhow!("Failed to locate sys creds path. Path={:?} Err={}", sys_path, e));
                        }
                    },
                    None => CredPaths {
                        host_creds_path: host_creds_file_path,
    ...
    pub async fn save_host_creds(
        ...
        Command::new("nsc")
            .arg(format!("generate creds --name user_host_{} --account {} > {}", self.host_pubkey, "WORKLOAD", host_creds_file_name))
            .output()
        ...
        if let Some(sys_pubkey) = self.local_sys_pubkey.as_ref() {
            ...
            Command::new("nsc")
                .arg(format!("generate creds --name user_host_{} --account {} > {}", sys_pubkey, "SYS", file_name))
                .output()
                .context("Failed to add new operator signing key on hosting agent")?;
        }
    
        self.to_owned().add_creds_paths(
            get_file_path_buf(host_creds_file_name),
            sys_creds_file_name
        )
    }
...
fn try_read_from_file(file_path: PathBuf) -> Result<Option<String>> {
    match file_path.try_exists() {
        Ok(link_is_ok) => {
            if !link_is_ok {
                return Err(anyhow!("Failed to read path {:?}. Found broken sym link.", file_path));
            }

            let mut file_content =
                File::open(&file_path).context(format!("Failed to open config file {:#?}", file_path))?;
