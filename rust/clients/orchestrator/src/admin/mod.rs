...
pub async fn run(
    ...
    admin_client_tasks.spawn({
        ...
        async move {
            ...
            services::workloads::run(admin_client_clone, db_client_clone)
                .await
    ...
    admin_client_tasks.spawn({
        ...
        async move {
            ...
            services::inventory::run(admin_client_clone, db_client)
                .await
                .map_err(|e: Box<dyn Error + Send + Sync + 'static>| {
                    OrchestratorError::Client(format!("Inventory client error: {:?}", e))
                })
        }
    });
    ...
    while let Some(result) = admin_client_tasks.join_next().await {
        match result {
            ...
            Ok(Err(e)) => log::warn!(
                "Admin client service (eg: workload or inventory) exited with error: {}",
                e
            ),
