...
use db_utils::schemas::WorkloadStateDiscriminants;
...
use nats_utils::{
    ...
    types::{
        HcHttpGwRequest, HcHttpGwResponse, JsServiceBuilder, ServiceConsumerBuilder, ServiceError,
    },
};
use reqwest::Method;
            ...
            while let Some((key, maybe_entry)) = final_stream.next().await {
                ...
                match desired_state {
                    WorkloadStateDiscriminants::Running => {
                        // TODO: ensure a worker is running
                        let subject_suffix =
                            HcHttpGwRequest::nats_subject_suffix(&installed_app_id);
                        ...
                        let consumer = nats_js_service
                            .add_consumer(
                                ...
                                .with_subject_prefix(installed_app_id)
                                .into(),
                            )
                            .await;
                    ...
                    | WorkloadStateDiscriminants::Unknown => {
                        // TODO: ensure no consumer exists
                    }
                }
            }
...
async fn hc_http_gw_consumer_handler(
    ...
    let local_request_url_suffix = incoming_request
        .get_checked_url(Some(http_gw_url_base.as_str()), &workload_id)
        .context(format!(
