...
use db_utils::schemas::{
    Workload, WorkloadDeployable, WorkloadDeployableHolochainDhtV1, WorkloadState,
        ...
        let (actual_status, status_payload) = match desired_state {
            WorkloadState::Installed | WorkloadState::Running => {
                ...
                let status_payload = match (desired_state, &workload.deployable) {
                    (WorkloadState::Running, WorkloadDeployable::HolochainDhtV1(boxed)) => {
                        ...
                        // TODO(clarify): try to call the init zome fn in every cell of the installed happ?
