...
mod tests {
    ...
    fn should_succeed_to_verify_access_token() {
        let result =
            verify_jwt::<AccessTokenClaims>(TOKEN.to_string(), SECRET.to_string()).unwrap();
    ...
    fn should_fail_to_verify_access_token() {
        ...
        let result = verify_jwt::<AccessTokenClaims>(invalid_token.to_string(), SECRET.to_string());
    ...
    fn should_fail_to_verify_access_token_with_invalid_secret() {
        ...
        let result = verify_jwt::<AccessTokenClaims>(TOKEN.to_string(), invalid_secret.to_string());
        assert!(result.is_err());
    }
}
