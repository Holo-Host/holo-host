...
pub fn verify_refresh_token(token: &str, secret: &str) -> Result<RefreshTokenClaims, jsonwebtoken::errors::Error> {
    verify_jwt::<RefreshTokenClaims>(token, secret)
}

pub fn verify_jwt<T: DeserializeOwned>(token: &str, secret: &str) -> Result<T, jsonwebtoken::errors::Error> {
