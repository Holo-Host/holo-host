...
mod tests {
    ...
    fn should_succeed_to_verify_access_token() {
        let result =
            verify_jwt::<AccessTokenClaims>(TOKEN.to_string(), Some(SECRET.to_string())).unwrap();
    ...
    fn should_fail_to_verify_access_token() {
        ...
        let result =
            verify_jwt::<AccessTokenClaims>(invalid_token.to_string(), Some(SECRET.to_string()));
    ...
    fn should_fail_to_verify_access_token_with_invalid_secret() {
        ...
        let result =
            verify_jwt::<AccessTokenClaims>(TOKEN.to_string(), Some(invalid_secret.to_string()));
        assert!(result.is_err());
    }
}
