...
mod tests {
    ...
    use crate::{
        ...
        middleware,
        providers::database::schemas::log::{Log, LOG_COLLECTION_NAME},
        tests::utils::{
            get_app_config, get_cache, perform_integration_test, IntegrationTestResponse, WebData
        }
    };

    pub async fn build_test_request(req: TestRequest, web_data: WebData) -> IntegrationTestResponse {
    ...
    pub async fn should_push_logs_to_redis_when_api_receives_a_request() {
        ...
        let resp = build_test_request(req, WebData{
        ...
        let logs: Vec<String> = conn.lrange(
            LOG_COLLECTION_NAME.to_string(),
            0,
            100
        ).await.unwrap();
        let mut count = 0;
